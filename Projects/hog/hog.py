...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for i in range(1, 11, +1):
        get_average = make_averaged(roll_dice, samples_count)
        new = get_average(i,dice)
        if record < new:
            record = new
            result = i
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    new = boar_brawl(score, opponent_score)
    if new >= threshold:
        return 0
    else:
        return num_rolls
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    new = sus_update(0, score, opponent_score)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    for i in range(0, 3, +1):
        new = sus_update(i, score, opponent_score)
        if new >= GOAL:
            return i
    get_average = make_averaged(roll_dice, 10)
    get_average_boar = make_averaged(boar_brawl, 10)
    new_0 = get_average_boar(score, opponent_score)
    new_6 = get_average(6)
    if new_0 >= new_6:
        return 0 
    return 6
